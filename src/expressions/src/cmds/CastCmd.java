...
public class CastCmd implements CommandExecutor {
	...
	public CastCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if (l.equalsIgnoreCase("cast")) {
			if ((s instanceof Player)) {
				Player p = (Player) s;
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Cast.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main
							.broadcast(p.getDisplayName() + this.main.getCmdMsg("Cast.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
