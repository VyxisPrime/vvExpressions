...
public class FlipCmd implements CommandExecutor {
	...
	public FlipCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if ((l.equalsIgnoreCase("flip")) && ((s instanceof Player))) {
			Player p = (Player) s;
			if (p.hasPermission("expressions.express.mid")) {
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Flip.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main
							.broadcast(p.getDisplayName() + this.main.getCmdMsg("Flip.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
