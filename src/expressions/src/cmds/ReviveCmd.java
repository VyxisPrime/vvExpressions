...
public class ReviveCmd implements CommandExecutor {
	...
	public ReviveCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if (l.equalsIgnoreCase("revive")) {
			if ((s instanceof Player)) {
				Player p = (Player) s;
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Revive.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main.broadcast(
							p.getDisplayName() + this.main.getCmdMsg("Revive.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
