...
public class HugCmd implements CommandExecutor {
	...
	public HugCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if ((l.equalsIgnoreCase("hug")) && ((s instanceof Player))) {
			Player p = (Player) s;
			if (p.hasPermission("expressions.express.low")) {
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Hug.single"));
				...
				if (args.length == 1) {
					...
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Hug.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
