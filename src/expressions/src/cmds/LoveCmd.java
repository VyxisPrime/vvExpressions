...
public class LoveCmd implements CommandExecutor {
	...
	public LoveCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if ((l.equalsIgnoreCase("love")) && ((s instanceof Player))) {
			Player p = (Player) s;
			if (p.hasPermission("expressions.express.low")) {
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Love.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main
							.broadcast(p.getDisplayName() + this.main.getCmdMsg("Love.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
