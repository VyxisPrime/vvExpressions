...
public class AnnoyCmd implements CommandExecutor {

	Main main = Main.getInstance();

	public AnnoyCmd(Main main) {
		this.main = main;
	}

	@Override
	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if ((l.equalsIgnoreCase("annoy")) && ((s instanceof Player))) {
			Player p = (Player) s;
			if (p.hasPermission("expressions.express.mid")) {
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Annoy.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main.broadcast(
							p.getDisplayName() + this.main.getCmdMsg("Annoy.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
