...
public class DanceCmd implements CommandExecutor {
	Main main = Main.getInstance();

	public DanceCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if ((l.equalsIgnoreCase("dance")) && ((s instanceof Player))) {
			Player p = (Player) s;
			if (p.hasPermission("expressions.express.low")) {
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Dance.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main.broadcast(
							p.getDisplayName() + this.main.getCmdMsg("Dance.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
