...
public class PokeCmd implements CommandExecutor {
	...
	public PokeCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if (l.equalsIgnoreCase("poke")) {
			if ((s instanceof Player)) {
				Player p = (Player) s;
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Poke.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main
							.broadcast(p.getDisplayName() + this.main.getCmdMsg("Poke.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
