...
public class PunchCmd implements CommandExecutor {
	...
	public PunchCmd(Main main) {
		this.main = main;
	}

	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		if (l.equalsIgnoreCase("punch")) {
			if ((s instanceof Player)) {
				Player p = (Player) s;
				if (args.length == 0) {
					this.main.broadcast(p.getDisplayName() + this.main.getCmdMsg("Punch.single"));
					return true;
				}
				if (args.length == 1) {
					...
					this.main.broadcast(
							p.getDisplayName() + this.main.getCmdMsg("Punch.targeted") + tp.getDisplayName());
					return true;
				}
			} else {
